...
// chat-service/handlers/websocket.go (updated)
package handlers
...
func handleLoadChat(userID, otherID int, before string, conn *websocket.Conn) {
	...
	userQuery := `SELECT name, email FROM chat_users WHERE id = $1`
	db.DB.QueryRow(userQuery, otherID).Scan(&userName, &userEmail)
	...
	if senderConn, ok := connections[otherID]; ok {
